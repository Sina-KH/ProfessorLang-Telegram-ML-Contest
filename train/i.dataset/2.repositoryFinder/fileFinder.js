...
async function loadRepos() {
    await findAndRemoveDuplicateContents('./files/')
        ...
        for (let i = 1; i <= 50; i++) {
            try {
                ...
                let config = {
                    ...
                    url: 'https://api.github.com/search/code?q=tl+extension:' + ext.forExtension + '&page=' + i,
                    headers: {
                        ...
                        if (item.path.endsWith('.' + ext.forExtension)) {
                            ...
                            if (responseItemData.data?.download_url) {
                                ...
                                download(responseItemData.data.download_url,
                                    './files/' + new Date().getTime() + '__' + item.path.split('/').join('__'))
                            }
                        }
                    } catch (e) {
                        console.log(e)
